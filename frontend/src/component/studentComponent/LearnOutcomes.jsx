...
import { getLearningOutcomesByUserId } from "../../services/EnrollmentService";
...
const LearnOutcomes = () => {
    ...
    useEffect(() => {
        const fetchLearnOutcomes = async () => {
            try {
                const response = await getLearningOutcomesByUserId(JSON.parse(localStorage.getItem('loginResponse')).id);
                    ...
                    <tbody>
                        {learnOutcomes.map((learnOutcome, index) => (
                            <tr key={index} className="overflow-hidden text-center odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                ...
                                <td className="px-6 py-4">
                                    {learnOutcome.excusedAbsenceCount}
                                </td>
                            </tr>
                        ))}
